...
The compiled bricklibrary.inline_gui binaries (.xdl64 for Windows, .xlib for
macOS) are committed under native/builds/<platform>/ so EITHER machine can
...
`write` records a sha256 over the module's source/ + project/ files into
...
Usage:
    python3 tools/native_stamp.py write native/builds/macos_arm64/bricklibrary.inline_gui
    python3 tools/native_stamp.py check native/builds/macos_arm64/bricklibrary.inline_gui
...
NATIVE_SRC = os.path.join(REPO_ROOT, "native", "bricklibrary.inline_gui")
